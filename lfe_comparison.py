...
def stable(ress, test, labels):  # ress is training data
    ...
    for i in range(0, len(val)):
        ...
        if r > eta_2:  # This is eta for stability selection
            finale.append(s)
...
if __name__ == '__main__':
    ...
    timings = []
    memory = []
    for i, dataset_info in pd.read_csv('datasets/lfe_datasets.csv').to_dict('index').items():
        clean()
        dataset = dataset_info['Dataset']
        print(dataset)
        start_time = time.time()
        memory_before = psutil.Process().memory_info().rss
        df = pd.read_csv('../../data/lfe_datasets/{}.csv'.format(dataset))
        X = df[df.columns[:-1]]
        y = df[df.columns[-1]]
        ...
        for fold, (train_index, test_index) in enumerate(StratifiedKFold(
            ...
            time_taken_per_fold = '{:.2f}'.format(time.time()-start_time_per_fold)
            memory_used_per_fold = '{:.2f}'.format(abs(psutil.Process().memory_info().rss - memory_before_per_fold) / (1024 * 1024))
            print 'F1: {} | Time: {} | Memory: {}'.format(f1, time_taken_per_fold, memory_used_per_fold)
            print 'fold-{} done.'.format(fold)
            print '{}'.format('_' * 80)

        time_taken = '{:.2f}'.format(time.time()-start_time)
        memory_usage = '{:.2f}'.format(abs(psutil.Process().memory_info().rss - memory_before) / (1024 * 1024))

        f1_scores = {k: '{:.3f}'.format(v/N_FOLDS) for k, v in f1_scores.items()}

        datasets.append(dataset)
        results.append(float(f1_scores.values()[0]))
        timings.append(time_taken)
        memory.append(memory_usage)

        result_df = pd.DataFrame({'Dataset': datasets, 'F1: AutoLearn': results, 'Time: AutoLearn': timings, 'Memory: AutoLearn': memory})
        result_df = result_df[['Dataset', 'F1: AutoLearn', 'Time: AutoLearn', 'Memory: AutoLearn']]
        result_df.to_csv('results/lfe_comparison.csv', index=False)
        print result_df
